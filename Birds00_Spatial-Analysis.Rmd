...
All the information I need is within my "site_info.txt" file. All site related information are stored inside (location, ids, name, etc.). I use a classical data.frame and set the site_ids as rownames, this works better with the spatial packages than a tidy workflow.  

```{r results='hide', message=FALSE}
site_info <- read.delim("20190515_site_infos.csv")
...
Now I have to extract the "data" from my 36 little landscape. Basically I want to now how many pixels (size is 10 x 10 m) belong to each landuse class. With that I can later claculate proportional cover. I can do that with the `extract()` function from the raster package. Note that there is another `extract()` in the tidyr package. 
...
Next tobasic pixel counts I also want to have the proportions of each class. For this I slightly tweak the functions above.
...
Since my sites are along an urbanization gradient I would like to now how far they are away from a central point within the city. I specified this point via Qgis. I choose the "Anil Kumble Circle" on the "MG road" as central point. 

```{r results='hide'}
centralP <- readOGR(dsn="P:/Science/Projekte/Bangalore/GIS/raw_data","central_point")
```
...
Now it is super simple to calculate the distance matrix. I also devide by 1000 to get the distances in km.
...
Lastly, I can export my output table. I use the R's own .rds formart, which I find easy to work with. csv files tend to have issues when imported (i.e. numbers to factors)
